...
const listAccounts = async () => {
    ...
    console.log(entry);
    accounts.innerHTML = entry.map(listTemplate).join("");
    /*
    accounts.innerHTML = `
    <li>
      <h3>${entry.user.username}</h3>
      <p>${entry.user._id}</p>
      <p>${entry.user.money}</p>
      <input type="number" name="amount" id="amount" step="100" min="0" required>
      <button data-function="deposit" data-postid="${entry.user._id}">Deposit</button>
      <button data-function="withdraw" data-postid="${entry.user._id}">Withdraw</button>
      <button data-function="delete" data-postid="${entry.user._id}">Remove Account</button>
    </li>
  `;*/
